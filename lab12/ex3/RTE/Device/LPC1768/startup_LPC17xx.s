...
sequentialSearch PROC
				...
				sub Inout1,#twowordsize
				sub Inout2,#twowordsize
				mov Price_Index,Inout1
outerloop				
				ldr Temp1,[Inout2,#twowordsize]! ;	Item_list
				mov Inout1,Price_Index 			 ;	reset price list index to first place
innerloop		
				ldr Temp2,[Inout1,#twowordsize]! ;	Price_list
				add countrN,#1
				cmp countrN,#n
				beq finishloop
				cmp Temp1,Temp2
...
founditem	    
				mov countrN,#0	
				ldr Temp1,[Inout1,#wordsize] 	
				ldr Temp2,[Inout2,#wordsize]	
...
founditemB				
				ldr Temp1,[Inout1,#wordsize] 	;	Read value of price 
